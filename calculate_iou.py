    ...
    for _, df1_dict in df1.iterrows():
        ...
        for _, df2_dict in df2.iterrows():
            ...
            if iou >= max_iou:
                ...
                max_iou = iou
        iou_scores.append({"df1_box": df1_box, "df2_box": best_df2_box, "iou": max_iou})
...
if __name__ == "__main__":
    ...
    patch_overlap_parameters = [0, 0.01, 0.02]
    ...
    for overlap in patch_overlap_parameters:
        for threshold in iou_threshold_parameters:
            for patch_size in patch_size_parameters:
                logging.info(
                    f"Testing with overlap {overlap}, threshold {threshold}, and patch size {patch_size}"
                )
                ...
                gt_pred_iou = match_boxes_and_compute_iou(
                    ground_truth_df, predictions_df
                )
                logging.info(f"Ground truth compared to Predictions IoU: {gt_pred_iou}")

                # Get the average IoU of each of the boxes in the prediction, compared to the ground truth
                pred_gt_iou = match_boxes_and_compute_iou(
                    predictions_df, ground_truth_df
                )
                logging.info(f"Predictions compared to Ground Truth IoU: {pred_gt_iou}")

                # Get the complete average IOU
                average_iou = (gt_pred_iou + pred_gt_iou) / 2
                logging.info("Average IoU:")
                logging.info(average_iou)

                params_iou_list.append(
                    {
                        "overlap": overlap,
                        "threshold": threshold,
                        "patch_size": patch_size,
    ...
    logging.info(f"results: {str(params_iou_list)}")
    ...
    logging.info(f"max iou params: {str(max_iou_dict)}")

    optimal_prediction = model.predict_tile(
        sample_image_path,
        return_plot=True,
        patch_overlap=max_iou_dict["overlap"],
        iou_threshold=max_iou_dict["threshold"],
        patch_size=max_iou_dict["patch_size"],
    )
